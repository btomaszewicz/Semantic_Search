...
def build_dictionary_in_chunks(csv_file,chunksize=100):

    dictionary = corpora.Dictionary()

    for chunk in load_tokenized_csv(csv_file, chunksize):
        # Add tokenized plots to dictionary
    ...
    dictionary.save("movie_dictionary.dict")
...
def build_tfidf(dictionary, csv_file, chunksize=100):
    corpus = (dictionary.doc2bow(text) for chunk in load_tokenized_csv(csv_file, chunksize) for text in chunk['wiki_plot_tokenized'])
    tfidf_model = TfidfModel(corpus, dictionary=dictionary)
    tfidf_model.save("tfidf_model.tfidf")  # Save to disk
    return movie_tfidf_model
